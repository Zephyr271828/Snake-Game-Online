...
def game_loop(sock):
    ...
    while playing:
            ...
            if event.type == KEYDOWN:
                if event.key == K_ESCAPE:
                    ...
                    if key_str is not None:
                        sock.send(json.dumps({"action" : "turn", "direction" : key_str}).encode("utf-8"))
                        self_snake.turn(key_str)

        if food.loc == self_snake.head:
            self_snake.eat_food = True
            food.update()
            sock.send(json.dumps({"action" : "food", "location" : food.loc}).encode("utf-8"))
        ...
        if self_snake.die(other_snake):
            ...
            sock.send(json.dumps({"action" : "quit"}).encode("utf-8"))
